...
export async function fetchTable(tableId, fields, { filterByFormula } = {}) {
  const params = fields.map(f => `fields[]=${encodeURIComponent(f)}`).join('&');
  const filterParam = filterByFormula ? `&filterByFormula=${encodeURIComponent(filterByFormula)}` : '';
  const allRecords = [];
  let offset = null;
  do {
    const url = `/.netlify/functions/cc-airtable/${BASE}/${tableId}?${params}${filterParam}${offset ? `&offset=${offset}` : ''}`;
