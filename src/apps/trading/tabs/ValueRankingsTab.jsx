...
const METRICS = [
  { key: "roiUnlimited",   label: "ROI Unlimited",    field: "ROI Ratio (Unlimited)" },
  { key: "roiSinglePayout", label: "ROI Single Payout", field: "ROI Ratio (Single Payout)" },
  { key: "roiReset",       label: "ROI Reset",         field: "ROI Ratio (Account Reset)" },
];
...
export function ValueRankingsTab() {
  ...
  const { perfTypes } = data;

  // Only include records with a value for the active metric
  const scored = perfTypes
  ...
  return (
    <div>
            ...
            {providers.map(dp => (
              <div key={dp}>
                ...
                {byProvider[dp].map(item => {
                  ...
                  return (
                    <div key={item.id} style={{ background: "#111827", border: `1px solid ${val >= 1.0 ? color + "44" : "#1f2937"}`, borderRadius: 6, padding: "6px 8px", marginBottom: 3 }}>
      ...
      {/* Section B: Top 20 ranked list */}
      <div>
        {sectionLabel("Top 20 Overall")}
        {top20.length === 0 ? (
          ...
          <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 10, overflow: "hidden", maxWidth: 640 }}>
            ...
            {top20.map((item, idx) => {
              ...
              const color = roiColor(val);
              return (
                <div key={item.id} style={{ display: "grid", gridTemplateColumns: "44px 1fr 140px 90px", borderBottom: idx < top20.length - 1 ? "1px solid #1f2937" : "none" }}>
